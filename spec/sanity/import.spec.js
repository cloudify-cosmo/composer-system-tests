...
function getGrid() {
  return element(by.css('#gridContainer'));
}

describe('Composer sanity', function() {
  ...
  describe('login', function() {
    ...
    it('should be successful', () => {
      expect($$('.userContainer').count()).toBe(1);
    });

    it('should drag', function() {
      var el = getStencil('Compute');
      var grid = getGrid();
      browser.actions().dragAndDrop(el, grid).perform();
      browser.actions().dragAndDrop(el, grid).perform();
      browser.actions().dragAndDrop(el, grid).perform();
  ...
  // xdescribe('folder behavior', function () {
